...
go build -o k8s-scale-test main.go
...
docker build -t wmdailey/k8s-scale-test:latest .
...
docker push wmdailey/k8s-scale-test:latest
...
image: wmdailey/scale-test:latest
You can now apply your Deployment and HP

# Step 5: Setup

kubectl apply -f scale-test-deploy.yaml -f scale-test-hpa.yaml
kubectl port-forward service/scale-test-svc 8080:8080
...
# 3.  **Test CPU Scaling:** Send a request that uses more than the target utilization (70% of 250m request):
    ...
    curl http://<SERVICE>:8080/load?type=cpu&value=60
...
# 4.  **Test Memory Scaling:** Send a request that uses memory beyond the target utilization (80% of 256Mi request is 204.8Mi).
    ...
    curl http://<SERVICE>:8080/load?type=memory&value=512
...
# 5.  **Check Release:**
    ...
    curl http://<SERVICE>:8080/status?action=release
