...
# Author: WKD
# Description: Queries Kubernetes to report the status of all Horizontal Pod Autoscalers (HPAs)
...
# Function to report the HPA status (Now first in the output)
report_hpa_status() {
#    echo "HORIZONTAL POD AUTOSCALER (HPA) STATUS in namespace: '$NAMESPACE'"
#    echo "----------------------------------------------------------------"

    # Run kubectl get hpa and filter out errors if namespace is invalid/empty
    HPA_OUTPUT=$(kubectl get hpa -n "$NAMESPACE" 2>/dev/null)

    if [ -z "$HPA_OUTPUT" ]; then
        echo "No Horizontal Pod Autoscalers (HPA) found in this namespace."
    else
        echo "$HPA_OUTPUT"
    fi
}
...
while true; do
    ...
    # HPA status is now reported first
    report_hpa_status
    echo "" 
